class WeatherApp {
  ...
  attachEventListeners() {
    ...
    this.iconLegendClose.addEventListener("click", () =>
      this.closeIconLegend()
    );
    this.iconLegendOverlay.addEventListener("click", () =>
      this.closeIconLegend()
    );

    // Demo city click handlers
  ...
  displayWeather(data, cityName) {
    ...
    const iconClass = this.getWeatherIconClass(
      weather.weather[0].main,
      weather.weather[0].description
    );
    ...
    mainIconElement.title = this.getWeatherIconDescription(
      weather.weather[0].main,
      weather.weather[0].description
    );
    ...
    document.getElementById(
      "temp-range"
    ).textContent = `H: ${tempMax}° L: ${tempMin}°`;
    ...
    conditionIconElement.title = this.getWeatherIconDescription(
      weather.weather[0].main,
      weather.weather[0].description
    );
      ...
      button.addEventListener("click", async () => {
        ...
        try {
          const res = await fetch(
            `/api/weather?lat=${match.lat}&lon=${match.lon}`
          );
          if (!res.ok) throw new Error(await res.text());
          const data = await res.json();
          this.displayWeather(
  ...
  getWeatherIconClass(condition, description = "") {
    ...
    if (conditionLower === "clear") {
      ...
      descriptionLower.includes("thunderstorm")
    ) {
      ...
      descriptionLower.includes("wind") ||
      conditionLower.includes("wind") ||
      descriptionLower.includes("squall") ||
      conditionLower === "tornado"
    ) {
  ...
  displayForecast(forecastData) {
    ...
    dailyForecasts.forEach((day, index) => {
      ...
      const iconTooltip = this.getWeatherIconDescription(
        day.weather[0].main,
        day.weather[0].description
      );
